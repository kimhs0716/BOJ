...
vl divisors(vector<pll> &fac) {
    vl ret {1};
    for(auto &[p, e] : fac){
        int sz = ret.size();
        ll mul = 1;
        for(int exp = 1; exp <= e; ++exp){
            mul *= p;
            for(int i = 0; i < sz; ++i){
                ret.push_back(ret[i] * mul);
            }
        }
    }
    return ret;
...
void solve(ll testcase){
    ...
    if (a == 0) {
        ...
        ll g = gcd(abs(b), abs(c));
        if (d % g == 0) cout << "INFINITY\n";
        else cout << "0\n";
        return;
    }
    vector<pll> ans;
    ...
    vector<pll> fac2;
    for (auto p: fac) {
        if (fac2.empty() || fac2.back().first != p) {
            fac2.emplace_back(p, 1);
        }
        else {
            fac2.back().second++;
        }
