...
Fraction p(ll r, ll w) {
    ...
    if (cache[r][w].ja > -1) return cache[r][w];
    cache[r][w] = p(r, w-2) * Fraction(w, r+w) * Fraction(w-1, r+w-1);
...
void solve(ll testcase) {
    ll i, j;
    long long r, w; cin>>r>>w;
    Fraction ans=p(r, w);
    cout<<(long long)ans.ja<<'/'<<(long long)ans.mo<<endl;
}
