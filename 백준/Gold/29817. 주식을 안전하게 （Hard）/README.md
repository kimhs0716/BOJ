# [Gold I] 주식을 안전하게 (Hard) - 29817 

[문제 링크](https://www.acmicpc.net/problem/29817) 

### 성능 요약

메모리: 2428 KB, 시간: 224 ms
...
<p><strong>주의: 주식을 안전하게(Hard) 문제는 주식을 안전하게(Easy) 문제와 구해야 하는 값이 다르다.</strong></p>
...
<p>김한양의 경제학과 친구 한누리는 김한양이 만든 이 기준만으로는 인공지능이 안전하게 주식 투자를 하고 있는지 확인할 수 없다고 생각한다. 이를 김한양에게 설명하기 위해 $k$일차의 기준치 $D_k$의 값을 직접 계산하는 프로그램을 여러분의 도움을 받아 만들었다.</p>

<p>그러나 한누리는 이 프로그램만 가지고 김한양에게 설명하기에는 아직 부족하다고 느꼈다. 이번에는 인공지능이 안전하게 주식 투자를 한 지 $k$일 차일 때, 통장에 남은 금액인 $M_k$의 최솟값을 계산하는 프로그램이 필요하다. 한누리를 또 도와주자!</p>

### 입력 

 <p>첫 줄에는 세 정수 $p(2≤p≤100)$, $c(1≤c≤100)$, $k(p≤k≤10^{18})$가 공백을 사이로 해서 주어진다.</p>

<p>다음 줄에는 $0$일 차부터 $p-1$일 차까지 각 일 차마다 통장에 있는 금액을 나타내는 $p$개의 정수 $M_0, \dots, M_{p-1}(-10^8≤M_i≤10^8)$이 공백을 사이로 해서 주어진다.</p>

### 출력 

 <p>인공지능이 안전하게 주식 투자를 한 지 $k$일 차일 때, 통장에 남은 금액인<strong> $M_k$의 최솟값을 $10^9+7$로 나눈 나머지</strong>를 출력한다.</p>
