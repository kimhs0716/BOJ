...
tree = SegTree(n, lambda x, y: tuple([x[i] + y[i] for i in (0, 1)]))

arr = list(map(int, input().split()))

for i in range(n):
    tree.update(i, ((arr[i] + 1) % 2, arr[i] % 2))
...
for _ in range(m):
    ...
    if a == 1:
        tree.update(b - 1, ((c + 1) % 2, c % 2))
        # tree.print()
    elif a == 2:
        print(tree.get(b - 1, c)[0])
    elif a == 3:
        print(tree.get(b - 1, c)[1])
