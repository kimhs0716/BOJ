...
void solve(ll testcase) {
    ll i, j, k;
    ll n, m; cin>>n>>m;
    vvl arr(n, vl(m)); cin>>arr;
    ll ans = 0;
    for (i=0;i<n;i++) ans += accumulate(arr[i].begin(), arr[i].end(), 0LL);
    Dinic dinic(n*m+2, n*m, n*m+1);
    for (i=0;i<n;i++) {
        for (j=0;j<m;j++) {
            if ((i+j)&1) dinic.add_edge(n*m, i*m+j, arr[i][j]);
            else dinic.add_edge(i*m+j, n*m+1, arr[i][j]);
        }
    }
    for (i=0;i<n-1;i++)
        for (j=0;j<m;j++) {
            if ((i+j)&1) dinic.add_edge(i*m+j, i*m+m+j, INF);
            else dinic.add_edge(i*m+m+j, i*m+j, INF);
        }
    for (i=0;i<n;i++)
        for (j=0;j<m-1;j++) {
            if ((i+j)&1) dinic.add_edge(i*m+j, i*m+j+1, INF);
            else dinic.add_edge(i*m+j+1, i*m+j, INF);
        }
