...
struct SEG {
    ...
    ll kth(ll node, ll l, ll r, ll k) {
        if (l == r) return l;
        ll m = (l + r) >> 1;
        if (seg[node<<1] >= k) return kth(node<<1, l, m, k);
        return kth(node<<1|1, m+1, r, k - seg[node<<1]);
    }
    ll find(ll tar) { return kth(1, 0, n-1, tar); }
};
...
void solve(ll testcase) {
    ...
    vl cnt(MAX+1);
    SEG seg(MAX+1);
    for (i=0;i<k-1;i++) seg.update(arr[i], ++cnt[arr[i]]);
    ll ans=0;
    for (i=k-1;i<n;i++) {
        seg.update(arr[i], ++cnt[arr[i]]);
        ans += seg.find((k+1)/2);
        // cout<<seg.find((k+1)/2)<<endl;
        seg.update(arr[i-k+1], --cnt[arr[i-k+1]]);
    }
    cout<<ans<<endl;
}
